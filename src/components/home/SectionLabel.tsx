    ...
    <div className={cn('flex items-end justify-between gap-3 mb-2.5', className)}>
      <div className="min-w-0">
        <div className="flex items-center gap-1.5">
          {Icon && <Icon className="w-3.5 h-3.5 text-muted-foreground/80" aria-hidden />}
          <h2 className="text-[14px] font-extrabold tracking-tight text-foreground/90 leading-none">
            {label}
          </h2>
          {count !== undefined && count > 0 && (
            <span className="text-[11px] font-semibold tabular-nums text-muted-foreground/75 leading-none ml-0.5">
              {count}
            </span>
          )}
        </div>
        {sublabel && (
          <p className="text-[11.5px] text-muted-foreground/80 leading-tight mt-1 truncate">
            {sublabel}
          </p>
        )}
      </div>
      {to && (
        <Link
          to={to}
          className="text-[11.5px] font-semibold text-muted-foreground/85 hover:text-foreground transition-colors inline-flex items-center gap-0.5 flex-shrink-0"
        >
